...
Floating point values cannot be reinterpreted directly as integrals, so we should handle floats and integrals in different ways. Instead of having ugly casts across the code, I worte a template that handles different cases through specialization, then a simple get/set interface on top of that:
...
and manipulating the value hold/viewed by the bitchunk:
...
I have the habit of implementing a weird technique just for fun. This time were tagged pointers. A month ago I was talking with a friend about how she could learn good C++, and as I usually do, I engouraged her to implement a `string` class. That way you learn about RAII, the rule of three/five, to hate raw memory management, and in the long term embrace the rule of zero by using `std::string` and not writting a custom destructor or assignment operator anymore. *"Except for special (i.e. freak and funny) purposes"* I said her.  

*"but you should note that the string implementation you know from the university, the simple dynamic array, is never used in real code because of performance considerations"*. That's the problem. Every CS grad knows how to write an string or a stack, but no one knows **how real world data structures work**. Teachers told much about Big O, but nothing about hardware. In my experience, most of them still think that [CPUs work like in the eighties](todo), this year I have even had a two hours discussion with my algorithms teacher because of some timings I did for an assignment: *"This should be wrong, if you do the math this says your CPU is executing 1.5 instructions per cycle on average. It's impossible to execute more than one instruction per cycle."*.

So I started to work on simple examples of the most common tricks for string implementations, such as SSO, copy on write, etc. Then an idea came to my mind: What about tagged pointers?
...
With the `bitchunk` template above, writting a simple wrapper for a tagged pointer was a matter of 3 minutes:
...
With the ugly iterators above the string class is completed with the classic range acessors, `operator[]`, etc:
...
Bitchunks were just an example of this, an idea that looks simple at the beginning, but takes a lot of hours to take it working everywhere the way you want.
